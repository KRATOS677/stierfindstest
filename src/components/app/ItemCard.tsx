import { MapPin, Calendar, User, Trash2, CheckCircle2, Hand, MessageSquare } from "lucide-react";
...
export const ItemCard = ({
  ...
  const canClaim = !isOwner && item.status === "unclaimed" && !isAdmin;
  ...
  return (
    <article className="bg-card rounded-xl overflow-hidden border border-border hover:border-primary/50 transition-all hover:-translate-y-1 hover:shadow-elevated group">
        ...
        <div className="absolute top-2 left-2 flex gap-1.5">
          <span
            className={`text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded-full ${
              item.type === "found" ? "bg-success text-white" : "bg-destructive text-white"
            }`}
          >
            {item.type}
          </span>
          <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded-full border ${statusStyles[item.status]}`}>
            {item.status}
          </span>
        </div>
      </div>
      ...
      <div className="p-4">
        <h3 className="font-bold text-foreground text-base mb-1 line-clamp-1">{item.name}</h3>
        {item.category && <p className="text-xs text-primary font-semibold uppercase tracking-wider mb-2">{item.category}</p>}
        ...
        <div className="space-y-1.5 text-xs text-muted-foreground border-t border-border pt-3">
          ...
          <div className="flex items-center gap-1.5">
            ...
            {item.user_id && currentUserId !== item.user_id ? (
              ...
              <span>{item.uploader_name || item.uploaded_by_name || "Admin"}{item.user_id && currentUserId === item.user_id ? " (You)" : ""}</span>
            )}
          </div>
        </div>

        {(canClaim || canDelete || canMarkReturned) && (
